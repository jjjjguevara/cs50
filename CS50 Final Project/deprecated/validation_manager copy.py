...
from .dtd.dtd_models import ValidationState
...
class ValidationManager:
    ...
    def __init__(
        ...
        self._pipelines: Dict[str, ValidationPipeline] = {}
        self._validators: Dict[str, Dict[str, Callable[..., ValidationResult]]] = {}
    ...
    def _init_validators(self) -> None:
            ...
            self._validators = {
                "content": {
                    "pattern": self._validate_with_patterns,
                    "schema": self._validate_with_schema
                },
                "config": {
                    "structure": self._validate_config_structure,
                    "dependencies": self._validate_config_dependencies
                },
                "metadata": {
                    "schema": self._validate_metadata_schema,
                    "inheritance": self._validate_metadata_inheritance
                }
            }
    ...
    def create_pipeline(
            ...
            Args:
                ...
                validators: List of (validator_name, validator_type) tuples
            """
            pipeline = ValidationPipeline()

            for validator_name, validator_type in validators:
                if validator := self._validators.get(pipeline_type, {}).get(validator_name):
                    pipeline.add_validator(validator_name, validator)
                else:
                    self.logger.warning(
                        f"Validator {validator_name} not found for {pipeline_type}"
                    )
    ...
    def register_validator(
        ...
        if pipeline_type not in self._validators:
            self._validators[pipeline_type] = {}
        self._validators[pipeline_type][validator_name] = validator
    ...
    def _validate_with_patterns(
        ...
        """Validate using registered patterns."""
        messages = []
    ...
    def register_dtd_pipeline(
        ...
        # Convert DTD validation methods to return ValidationResult
        ...
        # Register DTD validators
        self.register_validator("dtd", "structure", structure_validator)
        self.register_validator("dtd", "attributes", attributes_validator)

        # Create DTD pipeline
        self._pipelines["dtd"] = self.create_pipeline(
            "dtd",
            [("structure", "dtd"), ("attributes", "dtd")]
        )

    def register_pattern(self, pattern_id: str, pattern: ValidationPattern) -> None:
        """Register a validation pattern."""
        self._pattern_registry[pattern_id] = pattern

    def load_patterns(self, patterns_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for category, category_patterns in patterns.items():
                for pattern_id, pattern_def in category_patterns.items():
                    ...
                    self.register_pattern(full_id, pattern)

            # Load pattern groups
            self._pattern_groups = patterns_data.get("pattern_groups", {})

            # Load dependencies
            self._pattern_dependencies = patterns_data.get("dependencies", {})

            # Load conditional rules
            self._conditional_rules = patterns_data.get("conditional_rules", {})
    ...
    def validate(
        ...
        """
        try:
            if pipeline := self._validation_pipelines.get(validation_type):
                return pipeline.validate(content, context)
    ...
    def validate_content(
        ...
        # Get DTD validation results if available
        if dtd_results := element.metadata.get('dtd_validation'):
            if not dtd_results['is_valid']:
                messages.extend([
                    ValidationMessage(
                        path=element.id,
                        message=msg,
                    ...
                    ) for msg in dtd_results['messages']
                ])
