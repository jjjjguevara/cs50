...
class DTDValidationPattern(CachePattern):
    """Pattern for DTD validation results."""

    def generate_key(self, **kwargs: Any) -> str:
        """Generate cache key for DTD validation results."""
        element_id = kwargs.get('element_id')
        phase = kwargs.get('phase')
        dtd_name = kwargs.get('dtd_name', 'unknown')

        if not all([element_id, phase]):
            raise ValueError("Missing required parameters for DTD validation cache key")

        return f"dtd_validation:{element_id}:{phase}:{dtd_name}"

    def should_cache(self, value: Any) -> bool:
        """Determine if value should be cached."""
        # Cache if it's a ValidationResult or dict with validation data
        return isinstance(value, (dict, ValidationResult))

    def get_ttl(self) -> Optional[int]:
        """Get TTL for DTD validation results."""
        return 3600  # Cache for 1 hour by default


CACHE_PATTERNS: Dict[CacheEntryType, CachePattern] = {}

def init_cache_patterns() -> None:
    """Initialize cache pattern registry."""
    global CACHE_PATTERNS
    CACHE_PATTERNS = {
        CacheEntryType.DTD: DTDValidationPattern(
            CachePatternConfig(
